			...
			<div class="section group">
				...
				<div class="col-md-6 span_2_of_3">
					<div class="contact-form">
						...
						<form method="post" action="realpayment.jsp">
							<div>
								<% 
long ID = (long)session.getAttribute("idd");
UserDao dao = new UserDao();
User emp = dao.findUserById(ID);
%>

								<% String ff = (String)session.getAttribute("UserID"); %>

								<span><label>User ID</label></span> <span><input
									value="<%=emp.getUserID() %>" name="UserID" type="text" class="textbox" readonly ></span>
							</div>
							<% String ss  = (String)session.getAttribute("Name"); %>
							
							<span><label>Name</label></span> <span><input
									value="<%=emp.getName() %>" name="Name" type="text" class="textbox" readonly ></span>
									
							<% String ll = (String)session.getAttribute("Name"); %>
							<span><label>Lastname</label></span> <span><input
									value="<%=emp.getLastname() %>" name="Lastname" type="text" class="textbox" readonly ></span>
							<div>
								<% String pp = (String)session.getAttribute("Address"); %>
								<span><label>Address</label></span> <span><textarea
										 name="Adress" type="text" class="textbox" readonly><%=emp.getAddress() %> </textarea></span>
							</div>
							...
							<form method="post" action="edituser.jsp?id=<%=emp.getId()%>">
